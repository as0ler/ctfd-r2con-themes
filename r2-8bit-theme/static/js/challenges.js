...
function loadchals(cb) {
  $.get(script_root + "/api/v1/challenges", function(response) {
    ...
    for (var i = challenges.length - 1; i >= 0; i--) {
      ...
      if ($.inArray(challenges[i].category, categories) == -1 && challenges[i].category != 'Quiz') {
        ...
        if (category == 'Hard') {
            categoryrow.find(".category-header").append($('<i class="nes-icon coin is-large"></i>'));
            categoryrow.find(".category-header").append($('<i class="nes-icon coin is-large"></i>'));
            categoryrow.find(".category-header").append($('<i class="nes-icon coin is-large"></i>'));
        }
        if (category == 'Medium') {
    ...
    for (var i = challenges.length - 1; i >= 0; i--) {
      ...
      if ($.inArray(challenges[i].category, categories) == -1 && challenges[i].category == 'Quiz') {
        ...
        categoryrow
          .find(".category-header")
          .append($("<h3>" + category + "</h3>"));
            categoryrow.find(".category-header").append($('<img src="/themes/r2ctf/static/img/mariobox.gif" class="img-thumbnail" width="50" height="50"></img>'));
        $("#challenges-board").append(categoryrow);
      }
    }
